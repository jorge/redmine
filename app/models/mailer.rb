...
class Mailer < ActionMailer::Base
  ...
  def attachments_added(attachments)
    ...
    when 'Project'
      ...
      recipients = container.project.notified_users.select {
                     |user| user.allowed_to?(:view_files, container.project)
                   }.collect  {|u| u.mail}
    when 'Version'
  ...
  def account_activation_request(user)
    ...
    @user = user
    @url = url_for(:controller => 'users', :action => 'index', :status => User::STATUS_REGISTERED,
                   :sort_key => 'created_on', :sort_order => 'desc')
    mail "account_activation_request",
         :to => User.active.find(:all, :conditions => {:admin => true}).collect { |u| u.mail }.compact,
  ...
  def mail(method_name, attributes)
    ...
    if @author && @author.logged? && @author.pref[:no_self_notified]
      if attributes[:to]
        attributes[:to] = (attributes[:to].is_a?(Array) ? attributes[:to] : [attributes[:to]]) - [@author.mail]
      end
      if attributes[:cc]
        attributes[:cc] = (attributes[:cc].is_a?(Array) ? attributes[:cc] : [attributes[:cc]]) - [@author.mail]
      end
    end
